...
def build_horizon_simulations(
    ...
    if cfg.run_metric:
        ...
        metric_engines_map = build_metrics_engines(
            cfg=cfg, scenarios=scenarios)
    ...
    if pre_built_planners is None and not planner_cls.requires_scenario:
        ...
        planners = build_planners(
            cfg.planner, scenario=None, cache=planners_cache)
    else:
    ...
    for scenario in tqdm(scenarios, total=len(scenarios)):
        if planner_cls.requires_scenario:
            planners = build_planners(
                cfg.planner, scenario, cache=planners_cache)
        ...
        for planner in planners:
            ...
            # Metric Engine
            metric_engine = metric_engines_map.get(scenario.scenario_type,
                                                   None)
