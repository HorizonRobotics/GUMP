...
from casadi import DM, Opti, OptiSol, cos, diff, sin, fmod, tan, sumsqr, vertcat
...
class KinematicSolver:
    ...
    def __init__(self, trajectory_len: int, dt: float, wheel_base: float):
        """
    ...
    def _create_decision_variables(self) -> None:
        ...
        self.steering = self.control[0, :]
        self.accel = self.control[1, :]
        self.curvature = tan(self.steering)/ self.wheel_base
    ...
    def _set_dynamic_constraints(self) -> None:
        ...
        Set the system dynamics constraints as following:
          dx/dt = f(x,u) using the bicycle model.
        """
        state = self.state
        control = self.control
        dt = self.dt
        def propagate(x0, y0, h0, v0, dt, wheel_base, control_acc, control_steering):
            """
            Propagate the current dynamics and state using Kinetics Bicycle Model
            """
            v1 = v0 + control_acc * dt
            h1 = h0 + dt * v1 * tan(control_steering) / wheel_base
            h1 = h1 = fmod(h1 + np.pi, 2 * np.pi) - np.pi
        
            x1 = v1 * cos(h1) * dt + x0
            y1 = v1 * sin(h1) * dt + y0
            return x1, y1, h1, v1
            
        def process_bicycle(x: Sequence[float], u: Sequence[float], dt: float, wheel_base: float) -> Any:
            """Process for state propagation using bicycle model."""
            x1, y1, h1, v1 = propagate(x[0], x[1], x[2], x[3], dt, wheel_base, u[1], u[0])
            return vertcat(x1, y1, h1, v1)
    
        for k in range(self.trajectory_len):  # loop over control intervals
            # Runge-Kutta 4 integration using the bicycle model
            next_state = process_bicycle(state[:, k], control[:, k], dt, self.wheel_base)
    ...
    def _set_control_constraints(self) -> None:
        ...
        # curvature_limit = 1.0 / 2.0  # 1/m
        # self._optimizer.subject_to(self._optimizer.bounded(-curvature_limit, self.curvature, curvature_limit))
        steering_limit = 1.0  # rad
        self._optimizer.subject_to(self._optimizer.bounded(-steering_limit, self.steering, steering_limit))
    ...
    def _set_state_constraints(self) -> None:
        ...
        # max_lateral_accel = 6.0  # m/s^2, assumes circular motion acc_lat = speed^2 * curvature
