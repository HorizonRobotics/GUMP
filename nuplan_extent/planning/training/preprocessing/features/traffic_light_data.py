...
@dataclass
class TrafficLightData(AbstractModelFeature):
    """
    ...
    def to_device(self, device: torch.device) -> TrafficLightData:
        """Implemented. See interface."""
        return TrafficLightData(data=self.data)

    def to_feature_tensor(self) -> TrafficLightData:
        """Inherited, see superclass."""
        return TrafficLightData(data=self.data)

    @classmethod
    def deserialize(cls, data: Dict[str, Any]) -> TrafficLightData:
        """Implemented. See interface."""
        return TrafficLightData(data=data["data"])

    def unpack(self) -> List[TrafficLightData]:
        """Implemented. See interface."""
        return TrafficLightData(data=self.data)
