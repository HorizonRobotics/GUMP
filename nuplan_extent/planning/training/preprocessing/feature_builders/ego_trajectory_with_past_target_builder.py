...
class EgoTrajectoryWithPastTargetBuilder(AbstractTargetBuilder):
    """Trajectory builders constructed the desired ego's trajectory from a scenario."""

    def __init__(self, future_trajectory_sampling: TrajectorySampling, current_step: int = 8) -> None:
        """
    ...
    def get_targets(self, scenario: AbstractScenario) -> Trajectory:
        ...
        trajectory_absolute_states = [current_absolute_state] + [state for state in trajectory_absolute_states]
        current_absolute_state = trajectory_absolute_states[self._current_step]
