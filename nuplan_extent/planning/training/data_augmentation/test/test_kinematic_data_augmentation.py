...
class TestKinematicAgentAugmentation(unittest.TestCase):
    ...
    def setUp(self) -> None:
        ...
        self.features = {}
        self.features['raster'] = HorizonRaster(np.zeros((224, 224, 18)))
        ...
        self.uniform_augmentor = RasterKinematicAgentAugmentor(
            N,
            dt,
            mean,
            std,
    ...
    def test_gaussian_augment(self) -> None:
        ...
        self.assertTrue(
            (aug_targets['trajectory'].data -
             self.gaussian_aug_targets_gt['trajectory'].data < 1e-04).all())

    def test_uniform_augment(self) -> None:
        """
        Test uniform augmentation.
        """
        _, aug_targets = self.uniform_augmentor.augment(
            self.features, self.targets)
        self.assertTrue(
            (abs(aug_targets['trajectory'].data -
                 self.uniform_aug_targets_gt['trajectory'].data) <= 0.1).all())
    ...
    def test_input_validation(self) -> None:
        ...
        """
        features = {'raster': None, 'test_feature': None}
        ...
        self.assertRaises(AssertionError, self.gaussian_augmentor.validate,
                          features, targets)
