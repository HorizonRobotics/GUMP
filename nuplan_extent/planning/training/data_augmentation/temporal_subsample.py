...
class TemporalSubsampleAugmentor(AbstractAugmentor):
    """
    ...
    def augment(
        ...
        if 'raster_sequence' in features:
            features['raster_sequence'].data = features['raster_sequence'].data[self._subsample_interval -
                                                                                1::self._subsample_interval]
        if 'trajectory' in targets:
            targets['trajectory'].data = targets['trajectory'].data[self._subsample_interval -
                                                                    1::self._subsample_interval]
