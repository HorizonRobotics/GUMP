...
class VisualizationKineticsGUMPCallback(pl.Callback):
    """
    ...
    def render_raster(self,
      ...
      # raster data
      raster_map = sum([(predictions['raster'][name][..., None].cpu().numpy() @ np.array(color)[None]) for name, color in self.raster_type.items()]) # B, H, W, C
    ...
    def _log_batch(
        ...
        # bsz, parallel, frame_len, agent_len, dim
        tokenized_arrays = predictions['tokenized_arrays'][:, 0]
        predicted_tokenized_arrays = predictions['predicted_tokenized_arrays'][:, 0]
        ...
        num_predict_steps = predicted_tokenized_arrays.shape[1]
        num_target_steps = num_predict_steps
        ...
        for batch_index in range(batch_size):
            ...
            for t in range(num_predict_steps):
                all_canvas[batch_index, t] = self.render_all_on_canvas_at_t(
                    tokenized_arrays[batch_index],
                    predicted_tokenized_arrays[batch_index],
                    all_canvas[batch_index, t],
                    frame_index=t
                )  # render all tokenized array
        
        target_canvas = rearrange(target_canvas, 'b t h w c -> b t c h w')
        predict_canvas = rearrange(predict_canvas, 'b t h w c -> b t c h w')
        all_canvas = rearrange(all_canvas, 'b t h w c -> b t c h w')
        ...
        logger.add_images(
            f'{prefix}_image_all',
            all_canvas.max(axis=1),
            global_step=training_step)
    
    def render_on_canvas_at_t(self, tokenized_arrays, canvas, frame_index):
        colors = plt.cm.rainbow(np.linspace(0, 1, 12))

        frame_len, agent_len, _ = tokenized_arrays.shape
        current_tokenized_arrays = tokenized_arrays[frame_index]
        current_tokenized_arrays = current_tokenized_arrays.view(kutils.NpKineticsSequenceArray)

        for i in range(len(current_tokenized_arrays)):
            # if tokenized_arrays:
            if current_tokenized_arrays[i].token_type == 3:
                continue
    ...
    def render_all_on_canvas_at_t(self, tokenized_arrays_target, tokenized_arrays_predict, canvas, frame_index):
        colors = plt.cm.rainbow(np.linspace(0, 1, 12))

        frame_len, agent_len, _ = tokenized_arrays_target.shape
        current_tokenized_arrays_target = tokenized_arrays_target[frame_index]
        current_tokenized_arrays_predict = tokenized_arrays_predict[frame_index]

        current_tokenized_arrays_target = current_tokenized_arrays_target.view(kutils.NpKineticsSequenceArray)
        current_tokenized_arrays_predict = current_tokenized_arrays_predict.view(kutils.NpKineticsSequenceArray)

        for i in range(len(current_tokenized_arrays_target)):
            # if tokenized_arrays:
            if current_tokenized_arrays_target[i].token_type != 3:
                x = current_tokenized_arrays_target[i].x
                y = current_tokenized_arrays_target[i].y
                w = current_tokenized_arrays_target[i].width
                l = current_tokenized_arrays_target[i].length
                # import pdb; pdb.set_trace()
                heading = current_tokenized_arrays_target[i].heading
                token_id = current_tokenized_arrays_target[i].track_id
                is_ego = current_tokenized_arrays_target[i].is_ego
                x, y, w, l, heading = self._transform_bbox(x, y, w, l, heading)
                color = colors[int(token_id) % len(colors)]  # ego color
                # if is_ego: shift the center to the rear axle
                canvas = draw_bev_bboxes([x, y, w, l, heading], canvas , color=color*255, fill=is_ego, l_shift=-1.461 if is_ego else 0.0)
            
            if current_tokenized_arrays_predict[i].token_type != 3:
                x = current_tokenized_arrays_predict[i].x
                y = current_tokenized_arrays_predict[i].y
                w = current_tokenized_arrays_predict[i].width
                l = current_tokenized_arrays_predict[i].length
                # import pdb; pdb.set_trace()
                heading = current_tokenized_arrays_predict[i].heading
                token_id = current_tokenized_arrays_predict[i].track_id
                is_ego = current_tokenized_arrays_predict[i].is_ego
                x, y, w, l, heading = self._transform_bbox(x, y, w, l, heading)
                color = colors[int(token_id) % len(colors)]  # ego color
                # if is_ego: shift the center to the rear axle
                canvas = draw_bev_bboxes([x, y, w, l, heading], canvas , color=color*255, fill=is_ego, l_shift=-1.461 if is_ego else 0.0)
